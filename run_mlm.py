...
class DataTrainingArguments:
    ...
    dataset_name: Optional[str] = field(
        metadata={"help": "The name of the dataset to use (via the datasets library)."}, default=None
    )
...
def main():
    # See all possible argumNoneents in src/transformers/training_args.py
    # or by passing the --help flag to this script.
    # We now keep distinct sets of args, for a cleaner separation of concerns.

    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, TrainingArguments))
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        # If we pass only one argutment to the script and it's the path to a json file,
    ...
    print("****RAW DATASETS COLUMN NAMES:", raw_datasets["train"][0])
